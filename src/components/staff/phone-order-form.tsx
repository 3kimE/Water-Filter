...
export function PhoneOrderForm({ products }: { products: PickItem[] }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        setOkMsg(`Commande ${res.id} créée — elle apparaît dans la liste à confirmer.`);
  ...
  if (!open) {
    return (
      ...
      >
        <PhoneCall className="h-4 w-4" /> Ajouter une commande (téléphone)
      </button>
    );
  }
    ...
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <h2 className="flex items-center gap-2 font-display font-bold text-ink">
          <PhoneCall className="h-5 w-5 text-brand-500" /> Commande par téléphone
        </h2>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <input required value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} className={input} placeholder="Nom du client" />
        <input required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className={input} placeholder="Téléphone (0XXXXXXXXX)" inputMode="tel" />
        <input required value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} className={input} placeholder="Ville" />
        <input required value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} className={input} placeholder="Adresse" />
      </div>
      <input value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} className={`${input} mt-3`} placeholder="Note (optionnel)" />

      {/* Products */}
      <div className="mt-4 space-y-2">
        <p className="text-sm font-medium text-ink">Produits</p>
        {rows.map((r, i) => (
      ...
      <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
        <span className="text-sm text-ink-soft">
          Sous-total produits : <b className="text-ink">{formatMAD(subtotal)}</b>
          <span className="block text-xs">Livraison calculée automatiquement.</span>
        </span>
        <button
          type="submit"
          disabled={pending}
